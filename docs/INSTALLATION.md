...
```bash
cordova plugin add @da-systems-external/cordova-plugin-push
```
...
```bash
cordova plugin add github:da-systems-external/cordova-plugin-push
```
...
```xml
<plugin name="@da-systems-external/cordova-plugin-push" spec="3.1.0" />
```
...
```json
{
  "cordova": {
    "plugins": {
      "@da-systems-external/cordova-plugin-push": {
...
When some other packages include `cordova-google-play-services` as a dependency, such as is the case with the `cordova-admob` and `cordova-plugin-analytics` plugins, it is impossible to also add the `@da-systems-external/cordova-plugin-push`, for the following error will rise during the build process:
...
```log
Installing "@da-systems-external/cordova-plugin-push" for ios
Failed to install '@da-systems-external/cordova-plugin-push':Error: pod: Command failed with exit code 1

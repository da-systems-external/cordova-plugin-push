...
public class FCMService extends FirebaseMessagingService implements PushConstants {
    ...
    public void onMessageReceived(RemoteMessage message) {
        ...
        if (extras != null && isAvailableSender(from)) {
            ...
            SharedPreferences prefs = applicationContext.getSharedPreferences(PushPlugin.ORG_APACHE_CORDOVA_PUSH,
    ...
    public void createNotification(Context context, Bundle extras) {
        ...
        mBuilder.setWhen(System.currentTimeMillis()).setContentTitle(fromHtml(extras.getString(TITLE)))
                .setTicker(fromHtml(extras.getString(TITLE))).setContentIntent(contentIntent)
                .setDeleteIntent(deleteIntent)
                .setAutoCancel(true);

        SharedPreferences prefs = context.getSharedPreferences(PushPlugin.ORG_APACHE_CORDOVA_PUSH,
                Context.MODE_PRIVATE);
            ...
            try {
                ...
                for (int i = 0; i < actionsArray.length(); i++) {
                    ...
                    NotificationCompat.Action.Builder actionBuilder = new NotificationCompat.Action.Builder(
                            getImageId(resources, action.optString(ICON, ""), packageName), action.getString(TITLE),
                            pIntent);
    ...
    private boolean isAvailableSender(String from) {
        SharedPreferences sharedPref = getApplicationContext().getSharedPreferences(PushPlugin.ORG_APACHE_CORDOVA_PUSH,
                Context.MODE_PRIVATE);
